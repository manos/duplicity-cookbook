...
%w{cleanup restore status expire}.each do |script|
  cookbook_file "/etc/duplicity/#{script}" do
    mode "0555"
  end
end

%w{backup full_backup}.each do |template|
  template "/etc/duplicity/#{template}" do
    mode "0555"
    variables :backup_root => node[:duplicity][:backup_root]
  end
end
